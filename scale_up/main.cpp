...
void algo_4(geodesic::GeodesicAlgorithmExact &algorithm){
 ...
 //       fp=fopen("POINT.C","r");
     ...
     //       if(fscanf(fp,"%d",&x[i]) != EOF)
            {
                GeoNode *n=new GeoNode(x[i],0);
                AllPOI.push_back(n);
                std::pair<int , GeoNode*> m(x[i], n);
                POIs.push_back(m);
 //               printf("x[%d]: %d\n",i,x[i]);
            }
        }
//       fclose(fp);
       ...
       getrusage(RUSAGE_SELF,&myTime_program_start);
	    ...
	    geodesic::SurfacePoint source(&mesh.vertices()[x[0]]);
        std::vector<geodesic::SurfacePoint> all_sources(1,source);
	    double const distance_limit = 0;
        geodesic::SurfacePoint target(&mesh.vertices()[x[1]]);
	    std::vector<geodesic::SurfacePoint> stop_points(1, target);
        for(i=2;i<poi;i++){
            stop_points.push_back(geodesic::SurfacePoint(&mesh.vertices()[x[i]]));
        }
