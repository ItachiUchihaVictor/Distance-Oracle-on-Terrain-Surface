...
geodesic::Mesh mymesh;
int width = 100; //mymesh.m_width
          //  /mymesh.m_shortest_edge*1.414;
int height = 100;// mymesh.m_height/mymesh.m_shortest_edge*1.414;
//stx::btree<int , int>* cells[width+1][height+1];
stx::btree<int , int>* cells[100][100];
//FibonacciHeap<stx::btree<int, int>* >::FibNode * arr[width+1][height+1];
FibonacciHeap<stx::btree<int, int>* >::FibNode * arr[100][100];
namespace geodesic{
				   ...
				   double max_propagation_distance)
{
   ...
   //     std::cout<<"queue distance:"<<queue_distance<<"max_propagation_distance:"<<max_propagation_distance<<std::endl;
		...
		if(!m_queue.empty()&&*m_queue.begin()!=NULL)m_queue.erase(m_queue.begin());
...
//            std::cout<<"edge v0: "<<edge->v0()->id();
            if(distance<0.5*max_propagation_distance){
                int ID = edge->v0()->id(); //(*(*fh.top()).begin()).first;
                if(fh.size()>2&&tem_tree.exists(ID)){

                    int x = ((mesh.vertices()[ID]).getx()-mesh.m_xmin)/(*bplusparenttree.begin()).second->radius/1.414;
                    int y = ((mesh.vertices()[ID]).gety()-mesh.m_ymin)/(*bplusparenttree.begin()).second->radius/1.414;
          //          std::cout << "decrease key: x, y cells[x][y].size arr[x][y]->getKey.size" << x << " " << y << " " << (*cells[x][y]).size() << " " << std::endl;
                    arr[x][y]->setKey(cells[x][y]);
  //                  if(arr[x][y]->getKey()==NULL){
//                        std::cout << "NULL Key()";
    //                }else{
        //                std::cout << arr[x][y]->getKey()->size() << std::endl; 
      ...
      //              }
		...
		if(queue_distance > distance)
		{
                    int ID = edge->v1()->id(); // (*(*fh.top()).begin()).first;
                if(fh.size()>2&&tem_tree.exists(ID)){
