...
void algo_1_2(geodesic::GeodesicAlgorithmExact &algorithm){
        ...
        setnode(rootNode, mymesh.m_xmin, mymesh.m_ymin, mymesh.m_width, mymesh.m_height);
    ...
    //        printf("%f %f\n",rootNode->pointArray[i].x,rootNode->pointArray[i].y);
      ...
      // Create the quadTree
       ...
       /* std::ostringstream cmd;
        ...
        if(algo_type==2)Space_preprocess=2*quadpairs.size()*sizeof(Node)+sizeof(mymesh)+sizeof(algorithm);
#ifndef WIN32
...
void algo_3(geodesic::GeodesicAlgorithmExact &algorithm){
       ...
       getrusage(RUSAGE_SELF,&myTime_program_start);
#endif
        BuildGraphTree(*rootGraph,mymesh,algorithm);
       ...
       /* std::ostringstream cmd;
        ...
        Space_preprocess=2*graphpairs.size()*sizeof(GraphNode)+sizeof(mymesh)+sizeof(algorithm);
#ifndef WIN32
       getrusage(RUSAGE_SELF,&myTime_preprocess_end);
...
//       fclose(fp);
       ...
       getrusage(RUSAGE_SELF,&myTime_program_start);
	    ...
	    geodesic::SurfacePoint source(&mymesh.vertices()[poilist[0]]);
        std::vector<geodesic::SurfacePoint> all_sources(1,source);
	    double const distance_limit = 0;
        geodesic::SurfacePoint target(&mymesh.vertices()[poilist[1]]);
	    std::vector<geodesic::SurfacePoint> stop_points(1, target);
        for(i=2;i<num_poi;i++){
            stop_points.push_back(geodesic::SurfacePoint(&mymesh.vertices()[poilist[i]]));
        }
	    algorithm.propagate(all_sources, distance_limit, &stop_points);
        for(i=1;i<num_poi;i++){
            geodesic::SurfacePoint p(&mymesh.vertices()[poilist[i]]);
...
//#ifndef WIN32
    //   getrusage(RUSAGE_SELF,&myTime_query_end);
       Time_dquery+=get_micro_time();//myTime_query_end.ru_utime.tv_usec-myTime_query_begin.ru_utime.tv_usec;
//#endif
...
//       errorbound_knn/=qtimes;
       ...
       std::ofstream disquerytime("greedy.txt", std::ios::out | std::ios::app );
...
int main(int argc, char **argv) 
{
	...
	mymesh.initialize_mesh_data(points, faces);		//create internal mesh data structure including edges

    geodesic::GeodesicAlgorithmExact algorithm(&mymesh);	//create exact algorithm for the mesh
    std::cout << s << std::endl;
