...
double distance_geo(Node *x, Node *y, geodesic::GeodesicAlgorithmExact &algorithm){
    if(x==y)return 0;
	geodesic::SurfacePoint source(&mymesh.vertices()[x->indexArray[0]]);		//create source 
	...
	geodesic::SurfacePoint target(&mymesh.vertices()[target_vertex_index]);		//create source 
...
double distance_geo(GraphNode &x, GraphNode &y, geodesic::GeodesicAlgorithmExact &algorithm){
    if(&x==&y)return 0;
	geodesic::SurfacePoint source(&mymesh.vertices()[x.index]);		//create source 
	std::vector<geodesic::SurfacePoint> all_sources(1,source);					//in general, there could be multiple sources, but now we have only one
...
double dia_geo(Node *x, geodesic::GeodesicAlgorithmExact &algorithm){
    ...
    double distance=0;
    double maxdis=0;
	geodesic::SurfacePoint target(&mymesh.vertices()[x->indexArray[1]]);		//create source 
	std::vector<geodesic::SurfacePoint> stop_points(1, target);	//stop propagation when the target is covered
    for(int i=2;i<x->size;i++){
        stop_points.push_back(geodesic::SurfacePoint(&mymesh.vertices()[x->indexArray[i]]));
    }
	algorithm.propagate(all_sources, distance_limit, &stop_points);	//"propagate(all_sources)" is also fine, but take more time because covers the whole mymesh
    for(int i=1;i<x->size;i++){
        geodesic::SurfacePoint p(&mymesh.vertices()[x->indexArray[i]]);
...
double dia_geo(GeoNode &x, geodesic::GeodesicAlgorithmExact &algorithm){
	...
	geodesic::SurfacePoint target(&mymesh.vertices()[x.children.front()->index]);		//create source 
	std::vector<geodesic::SurfacePoint> stop_points(1, target);	//stop propagation when the target is covered
    std::list<GeoNode*>::iterator ite=x.children.begin();
    for(ite++;ite!=x.children.end();ite++){
        stop_points.push_back(geodesic::SurfacePoint(&mymesh.vertices()[(*ite)->index]));
    }
	algorithm.propagate(all_sources, distance_limit, &stop_points);	//"propagate(all_sources)" is also fine, but take more time because covers the whole mymesh
    ite=x.children.begin();
    for(ite++;ite!=x.children.end();ite++){
        geodesic::SurfacePoint p(&mymesh.vertices()[(*ite)->index]);
        algorithm.best_source(p,distance);
        maxdis=std::max(distance,maxdis);
    }
...
double surface_knn(int index,int k){
    ...
    stop_points.clear();
	geodesic::GeodesicAlgorithmExact algorithm(&mymesh);	//create exact algorithm for the mymesh
	geodesic::SurfacePoint source(&mymesh.vertices()[index]);		//create source 
	std::vector<geodesic::SurfacePoint> all_sources(1,source);					//in general, there could be multiple sources, but now we have only one
    ...
    for(int i=0;i<poi;i++){
	    ...
	    geodesic::SurfacePoint target(&mymesh.vertices()[x[i]]);		//create source 
        stop_points.push_back(target);
    }
    fclose(fp);
	algorithm.propagate(all_sources, 0, &stop_points, k);	//"propagate(all_sources)" is also fine, but take more time because covers the whole mymesh
    std::list<double> klist;
    for(int i=0;i<poi;i++){
			geodesic::SurfacePoint p(&mymesh.vertices()[x[i]]);		
			double distance;
            algorithm.best_source(p,distance);
