...
double shortestpath_LA(int x, int y, geodesic::GeodesicAlgorithmExact& shortestpath, std::vector<geodesic::GeodesicAlgorithmSubdivision *> * landmarks){
...
void SP_Oracle(geodesic::GeodesicAlgorithmExact &algorithm, geodesic::GeodesicAlgorithmSubdivision &mano_algo){
        double radius=0;
        double distance;
      ...
      // fclose(fp);
       ...
       Time_preprocess += clock();
       Time_preprocess = Time_preprocess/CLOCKS_PER_SEC;
...
//#endif
       qbegin = clock();
       double distance_return=shortestpath_LA(qindex1, qindex2, algorithm, &landmarks); 
...
//       fclose(fp); 
       std::ofstream disquerytime("sp-oracle.txt", std::ios::out | std::ios::app );
        disquerytime << eps << " " << num_poi << " " << mesh.vertices().size() << " " << Time_preprocess << " " << getValue() <<  " "  <<  Time_dquery << " " << errorbound_dis << std::endl; 
}
/*-------------------------------------------------------------------------
 * ------------------------MAIN--------------------------------------------
 *  -------------------------------------------------------------------------
...
//    double ss = 1/eps;
    int lsize = ((double) mesh.vertices().size())/(500.0*(eps+0.05))+100;
    ...
    float constant = (std::pow(1.0/(eps+0.05), 1.5)+1.0);//*std::sqrt(1-std::cos(mesh.smallest_angle));
    for(int i=0;i<lsize;i++){
        landmarks[i] = new geodesic::GeodesicAlgorithmSubdivision(&mesh, constant);
	...
	geodesic::GeodesicAlgorithmSubdivision algo(&mesh,constant);	//with subdivision_level=0 this algorithm becomes Dijkstra, with subdivision_level->infinity it becomes exact
